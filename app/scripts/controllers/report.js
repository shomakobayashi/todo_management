...
/**
 * @ngdoc function
 * @name moveupApp.controller:ReportCtrl
 * @description
 * # rise
 * Controller of the rise
 */
 angular.module('moveupApp')
   .controller('ReportCtrl', ['$scope', '$http', 'ApiService','$location','$routeParams', function ($scope, $http, apiService, $location, $routeParams) {
     ...
     $scope.getData = function () {
       ...
       if($routeParams.type){
       apiService.getReportList($routeParams.type,$scope.pageSize, $scope.pageSize * ($routeParams.page-1), function onSuccess(resultList) {
         ...
         if($routeParams.page > 1){
           $scope.lastPage = '/report/' + $routeParams.type + '/' + (parseInt($routeParams.page) - 1) + '/';
         }
         if($routeParams.page < totalPage){
           $scope.nextPage = '/report/' + $routeParams.type + '/' + (parseInt($routeParams.page) + 1) + '/';
         }
     ...
     $scope.initPage = function(totalPage){
       ...
       for(var i=0;i<totalPage;i++){
         ...
         var htmlCode = '<a href="/report/'+$routeParams.type+'/'+ (i + 1) + '/">' +
