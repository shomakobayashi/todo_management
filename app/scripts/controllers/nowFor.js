...
/**
 * @ngdoc function
 * @name moveupApp.controller:NowForCtrl
 * @description
 * # rise
 * Controller of the rise
 */
 angular.module('moveupApp')
   .controller('NowForCtrl', ['$scope', '$http', 'ApiService', '$location','$routeParams','NgMap', function ($scope, $http, apiService, $location, $routeParams,NgMap) {
    $scope.hideFooter = true;
    $scope.nowForHide = true;
    $scope.shopSearchR = {};
    $scope.shopSearchR.now4 = '1';
    ...
    $scope.getData = function () {
      ...
      if($routeParams.type){
        apiService.getNow4List($routeParams.type,$scope.pageSize,  $scope.pageSize * ($routeParams.page-1), function onSuccess(resultList) {
          ...
          if($routeParams.page > 1){
            $scope.lastPage = '/nowFor/' + $routeParams.type + '/' + (parseInt($routeParams.page) - 1) + '/';
          }
          if($routeParams.page < totalPage){
            $scope.nextPage = '/nowFor/' + $routeParams.type + '/' + (parseInt($routeParams.page) + 1) + '/';
          }
    ...
    //   paginationService.showFirstPageContent($scope, $scope.pageObject,1);
    // });
    //
    ...
    $scope.markClick=function (event,coordinate1,coordinate2,index){
      var contentList = $scope.results[0].contentList;
      for(var i = 0 ; i < contentList.length; i++){
    ...
    $scope.searchR = function () {
      ...
      apiService.postNow4Search($scope.shopSearchR,function onSuccess(result) {
    ...
    $scope.initPage = function(totalPage){
      ...
      for(var i=0;i<totalPage;i++){
        ...
        var htmlCode = '<a href="/nowFor/'+$routeParams.type+'/'+ (i + 1) + '/">' +
