 ...
 */
 angular.module('moveupApp')
   .controller('ChangeMailCtrl', ['$scope', '$location', '$window', 'ApiService','$routeParams','ngDialog',function ($scope, $location, $window, apiService,$routeParams,ngDialog) {
     $scope.hideFooter = false;

     $scope.title = 'メールアドレス変更';
    ...
    $scope.postChangeMail = function () {
      ...
      apiService.postChangeMail($scope.mail, function onSuccess(result) {
        $location.path('/changeMailSend/');
    ...
    if($routeParams.token){
      apiService.getChangeMail($routeParams.token, function onSuccess(result) {
        $window.localStorage['registMail'] = result.mail;
        $location.path('/changeMailEnd');
