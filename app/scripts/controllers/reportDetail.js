...
/**
 * @ngdoc function
 * @name moveupApp.controller:ReportDetailCtrl
 * @description
 * # rise
 * Controller of the rise
 */
 angular.module('moveupApp')
   .controller('ReportDetailCtrl', ['$scope', '$http', 'ApiService', '$window', '$location','$routeParams','ngDialog', '$timeout',function ($scope, $http, apiService, $window, $location,$routeParams,ngDialog,$timeout) {
     ...
     if($routeParams.uuid){
      ...
      apiService.getReportDetail($routeParams.uuid, function onSuccess(resultList){
        $scope.hideFooter = false;
     ...
     $scope.onApplication = function(){
       $window.localStorage['reportDetailInfo'] = JSON.stringify($scope.results2);
